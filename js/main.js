...
(function () {
  ...
  spyLinks.forEach(function (link) {
    ...
    if (sec) spySections.push({ id: link.getAttribute('data-spy'), el: sec, link: link });
  });

  function setActive(id) {
    spyLinks.forEach(function (l) { l.classList.remove('is-active'); });
    var hit = spySections.find(function (s) { return s.id === id; });
  ...
  if (practiceBtn) {
    practiceBtn.addEventListener('click', function (e) {
      ...
      setTimeout(function () { practiceBtn.classList.remove('is-wip'); }, 900);
    });
  }
})();
